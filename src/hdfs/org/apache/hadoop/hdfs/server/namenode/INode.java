...
public abstract class INode implements Comparable<byte[]> {
	...
	protected byte[] name;
	// @author coke
	// the name for the cache list
	// if cache list !=null search the cachelist and it present the inode i link
	// in the ancester node.
	protected byte[] cacheName;
	protected INodeDirectory parent;
	// @ author coke
	// for the parent disk index
	...
	// @ author coke
	// for the full string permission info
	PermissionStatus permissionInfo;
	protected long modificationTime;
	protected long accessTime;
	// @ author coke
	// for the blocksmap operations
	...
	protected INode() {
		...
		previous = null;
		next = null;
		permissionInfo=new PermissionStatus();
	}
	...
	protected void setUser(String user) {
		...
		permissionInfo.setUsername(user);
	...
	protected void setGroup(String group) {
		...
		permissionInfo.setGroupname(group);
	...
	boolean removeNode() {
		// remove the Node from the cache list
		previous.next = next;
		if (parent == null) {
			return false;
		} else {
	...
	// @author cokez
	// modify for searching in cachelist
	public int compareTo(byte[] o) {
		if (cacheName != null)
			return compareBytes(cacheName, o);
		else
	 ...
	 * @author cokez
	 */
	boolean readIDisk2INode(FileChannel fc) throws IOException {
		parentIndex = Tools.readInt(fc);
		name = Tools.readFBytes(fc);
		modificationTime = Tools.readLong(fc);
		accessTime = Tools.readLong(fc);
		 
		permissionInfo.readPermission(fc);
		 
		
		// compatible with original version set it again
		setPermissionStatus(permissionInfo);
		return true;
	}

	

	boolean writeINode2Disk(FileChannel fc) throws IOException {
		Tools.writeInt(fc, parentIndex);
		
		Tools.writeFBytes(fc, name);
		Tools.writeLong(fc, modificationTime);
		Tools.writeLong(fc, accessTime);
	
		permissionInfo.writePermission(fc);
	...
	public abstract int writeDisk(IndexManagement im) throws IOException;
	
	public abstract boolean readDisk(IndexManagement im, int index)
			throws IOException;
	...
	public boolean setNext(INode inode) {
		next = inode;
		return true;
	}
	...
	public boolean setPrevious(INode inode) {
		previous = inode;
		return true;
	}

	public abstract boolean delete(IndexManagement im) throws IOException;

	public abstract boolean clean(IndexManagement im) throws IOException;

	public abstract boolean update(IndexManagement im) throws IOException;

	public static void setFSNamesystem(FSNamesystem fsNameystem) {
		namesystem = fsNameystem;
	}
