...
class FSDirectory implements FSConstants, Closeable {
	...
	void loadFSImage(Collection<File> dataDirs, Collection<File> editsDirs,
	...
	INodeFileUnderConstruction addFile(String path,
			PermissionStatus permissions, short replication,
			long preferredBlockSize, String clientName, String clientMachine,
			DatanodeDescriptor clientNode, long generationStamp)
			throws IOException {
		waitForReady();
		...
		INodeFileUnderConstruction newNode = new INodeFileUnderConstruction(
				permissions, replication, preferredBlockSize, modTime,
				clientName, clientMachine, clientNode);
		// TODO write the INodeFile to the disk
		...
		if (newNode == null) {
			NameNode.stateChangeLog.info("DIR* FSDirectory.addFile: "
					+ "failed to add " + path + " to the file system");
			return null;
		}
	...
	/**
	 * TODO author cokeZ not used so long
	 */
			...
			try {
				...
				if (newNode != null && blocks != null) {
					...
					for (int i = 0; i < nrBlocks; i++) {
						newF.setBlock(i,
								namesystem.blocksMap.addINode(blocks[i], newF));
					}
				}
			} catch (IOException e) {
				return null;
	...
	INodeDirectory addToParent(String src, INodeDirectory parentINode,
		...
		if (blocks == null) {
			if (nsQuota >= 0 || dsQuota >= 0) {
				newNode = new INodeDirectoryWithQuota(permissions,
	...
	boolean removeBlock(String path, INodeFileUnderConstruction fileNode,
		...
		synchronized (rootDir) {
			...
			NameNode.stateChangeLog.debug("DIR* FSDirectory.addFile: " + path
					+ " with " + block + " block is added to the file system");
		...
		synchronized (rootDir) {
			...
			if (srcInodes[srcInodes.length - 1] == null) {
				NameNode.stateChangeLog
						.warn("DIR* FSDirectory.unprotectedRenameTo: "
			...
			try {
				...
				dstChild = addChildNoQuotaCheck(dstInodes,
						dstInodes.length - 1, srcChild, -1, false);
				if (dstChild != null) {
					srcChild = null;
					if (NameNode.stateChangeLog.isDebugEnabled()) {
						NameNode.stateChangeLog
								.debug("DIR* FSDirectory.unprotectedRenameTo: "
										+ src + " is renamed to " + dst);
					}
					// update modification time of dst and the parent of src
					srcInodes[srcInodes.length - 2]
							.setModificationTime(timestamp);
					dstInodes[dstInodes.length - 2]
							.setModificationTime(timestamp);
					return true;
				}
			} finally {
				if (dstChild == null && srcChild != null) {
					...
					addChildNoQuotaCheck(srcInodes, srcInodes.length - 1,
							srcChild, -1, false);
				}
			}
			NameNode.stateChangeLog
	...
	int unprotectedDelete(String src, long modificationTime) {
		...
		synchronized (rootDir) {
			...
			if (targetNode == null) { // non-existent src
				NameNode.stateChangeLog
						.debug("DIR* FSDirectory.unprotectedDelete: "
								+ "failed to remove " + src
								+ " because it does not exist");
				return 0;
			} else if (inodes.length == 1) { // src is the root
				NameNode.stateChangeLog
						.warn("DIR* FSDirectory.unprotectedDelete: "
								+ "failed to remove "
								+ src
				...
				try {
					...
					FSNamesystem.diskIO.update(inodes[inodes.length - 2]);
					// delete the diskInfo @cokez
					FSNamesystem.diskIO.delete(targetNode);
					// GC all the blocks underneath the node.
					ArrayList<Block> v = new ArrayList<Block>();
					int filesRemoved = targetNode
							.collectSubtreeBlocksAndClear(v);
	 ...
	 */
	// TODO change it for finalizeInodeFileUnderConstruction
	// operate with its parent
	void replaceNode(String path, INodeFile oldnode, INodeFile newnode)
		...
		synchronized (rootDir) {
			...
			FSNamesystem.diskIO.clean(oldnode);
			...
			// TODO write the node to the disk in the inodeDir.addChild
//			FSNamesystem.diskIO.writeINode(newnode);
//			if (!newnode.isUnderConstruction()) {
//				newnode.next = FSNamesystem.cacheRoot.next;
//				newnode.previous = FSNamesystem.cacheRoot;
//				FSNamesystem.cacheRoot.next.previous = newnode;
//				FSNamesystem.cacheRoot.next = newnode;
//
...
//			}
	...
	DirectoryListing getListing(String src, byte[] startAfter) {
		...
		synchronized (rootDir) {
			...
			return new DirectoryListing(listing, totalNumChildren - startChild
					- numOfListing);
		}
	}
	...
	private void updateCountNoQuotaCheck(INode[] inodes, int numOfINodes,
		...
		try {
			...
			NameNode.LOG.warn(
					"FSDirectory.updateCountNoQuotaCheck - unexpected ", e);
		}
	}
	...
	private static String getFullPathName(INode[] inodes, int pos) {
		...
		for (int i = 1; i <= pos; i++) {
			fullPathName.append(Path.SEPARATOR_CHAR).append(
	...
	boolean mkdirs(String src, PermissionStatus permissions,
		...
		synchronized (rootDir) {
			...
			for (; i < inodes.length; i++) {
				...
				NameNode.stateChangeLog
						.debug("DIR* FSDirectory.mkdirs: created directory "
								+ cur);
			}
		}
		return true;
   ...
   */
	INode unprotectedMkdir(String src, PermissionStatus permissions,
		...
		synchronized (rootDir) {
			rootDir.getExistingPathINodes(components, inodes);
			unprotectedMkdir(inodes, inodes.length - 1,
					components[inodes.length - 1], permissions, false,
					timestamp);
	...
	private void unprotectedMkdir(INode[] inodes, int pos, byte[] name,
			PermissionStatus permission, boolean inheritPermission,
			long timestamp) throws QuotaExceededException {
	 ...
	 * if it violates quota limit TODO @author cokez add the write disk
	 */
	 ...
	 */
	private void verifyQuota(INode[] inodes, int pos, long nsDelta,
		...
		try {
			...
			for (; i >= 0; i--) {
				if (commonAncestor == inodes[i]) {
					// Moving an existing node. Stop checking for quota when
					// common
					// ancestor is reached
					return;
				}
	...
	private <T extends INode> T addChildNoQuotaCheck(INode[] pathComponents,
		...
		try {
			...
			NameNode.LOG.warn("FSDirectory.addChildNoQuotaCheck - unexpected",
					e);
		}
		return inode;
	}
	...
	private INode removeChild(INode[] pathComponents, int pos) {
		...
		if (removedNode != null) {
			...
			updateCountNoQuotaCheck(pathComponents, pos, -counts.getNsCount(),
					-counts.getDsCount());
		}
		return removedNode;
	}
   ...
   */
	...
	private static void updateCountForINodeWithQuota(INodeDirectory dir,
		...
		if (dir.isQuotaSet()) {
			...
			if ((dir.getNsQuota() >= 0 && counts.nsCount > dir.getNsQuota())
					|| (dir.getDsQuota() >= 0 && counts.dsCount > dir
							.getDsQuota())) {
		...
		if (targetNode == null) {
			...
			if (dirNode instanceof INodeDirectoryWithQuota) {
				...
				if (!dirNode.isQuotaSet()) {
					// will not come here for root because root's nsQuota is
					// always set
				...
				// quota
				INodeDirectoryWithQuota newNode = new INodeDirectoryWithQuota(
						nsQuota, dsQuota, dirNode);
				// non-root directory node; parent != null
				INodeDirectory parent = (INodeDirectory) inodes[inodes.length - 2];
				dirNode = newNode;
				parent.replaceChild(newNode);
			}
			return (oldNsQuota != nsQuota || oldDsQuota != dsQuota) ? dirNode
					: null;
		}
	}
	...
	boolean unprotectedSetTimes(String src, long mtime, long atime,
			boolean force) throws IOException {
		INodeFile inode = getFileINode(src);
		return unprotectedSetTimes(src, inode, mtime, atime, force);
	}

	private boolean unprotectedSetTimes(String src, INodeFile inode,
			long mtime, long atime, boolean force) throws IOException {
		...
		if (atime != -1) {
			...
			if (atime <= inodeTime + namesystem.getAccessTimePrecision()
					&& !force) {
				status = false;
			} else {
	...
	private static HdfsFileStatus createFileStatus(byte[] path, INode node) {
		...
		return new HdfsFileStatus(node.isDirectory() ? 0 : ((INodeFile) node)
				.computeContentSummary().getLength(), node.isDirectory(),
				node.isDirectory() ? 0 : ((INodeFile) node).getReplication(),
				node.isDirectory() ? 0 : ((INodeFile) node)
						.getPreferredBlockSize(), node.getModificationTime(),
				node.getAccessTime(), node.getFsPermission(),
				node.getUserName(), node.getGroupName(), path);
	}
