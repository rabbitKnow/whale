...
import org.apache.hadoop.hdfs.server.namenode.whalebase.DiskIO;
...
public class FSNamesystem implements FSConstants, FSNamesystemMBean,
	...
	// TODO author cokez
	public static DiskIO diskIO;
	public static INode cacheRoot;
	public static INode cacheLeast;// cacheLeast is a empty Inode
	 ...
	 */
	// @cokez
	public NavigableMap<String, DatanodeDescriptor> datanodeMap = new TreeMap<String, DatanodeDescriptor>();
	...
	// @cokez
	public static final Log MetaLog = LogFactory.getLog("MetaTest");
	Daemon cachethread = null;// cache monitor thread
	private long cacheRecheckInterval; // ?
	private volatile boolean fsRunning = true;
	long systemStart = 0;
	// The maximum number of INode in the memory
	private long maxINodeNum;
	...
	private void initialize(NameNode nn, Configuration conf) throws IOException {
		...
		// @cokez set the inode fsnamesystem reference
		INode.setFSNamesystem(this);
		...
		// @cokeZ not loadFSImage
		// TODO load the FSImage
		...
		// disk initialize
		diskIO = new DiskIO(conf);
		//recover the root 
		//true is recover ,false is write to disk
		boolean recoverable=diskIO.recoverRoot(dir.rootDir);
		MetaLog.info("recover the rootdir"+recoverable);
		...
		// @author cokez
		// memory cache
		this.cachethread = new Daemon(new CacheMonitor());
		cachethread.start();
		// clean the totalCacheNum
		totalCacheNum = 0;
		...
		// initialize the inode info
		// @cokez
		cacheRoot = new INodeDirectory();
		cacheLeast = new INodeDirectory();
		cacheRoot.next = cacheLeast;
		cacheLeast.previous = cacheRoot.next;
	}
	...
	private void setConfigurationParameters(Configuration conf)
		...
		// set for inode num in memory
		this.maxINodeNum = conf.getLong("dfs.inode.max", 10);
		if (minReplication <= 0)
		...
		// cokez for cache check time for test
		this.cacheRecheckInterval = conf.getInt("dfs.cacheCheck.interval", 3) * 1000L;
		// default limit
	...
	// TODO @author cokez wait to init the versionID
	...
	private synchronized void startFileInternal(String src,
		...
		try {
			...
			if (append) {
				...
				//
				// Change it for preserve the cons inode info
	...
	/**
	 * 
	 * TODO store the inodefile metadata in the disk
	 * 
	 * @author cokez
	 * @param src
	 * @param pendingFile
	...
	private void finalizeINodeFileUnderConstruction(String src,
		...
		// link it the cache list locklist?
   ...
   */
	...
	class SafeModeInfo {
		...
		synchronized void leave(boolean checkForUpgrades) {
			...
			// TODO it is not necessory to check block
	...
	private long nextGenerationStamp() {
		...
		// TODO delete the interaction operator with Log
		...
		public void run() {
			while (fsRunning) {
				try {
					...
					if (totalCacheNum > maxINodeNum && !reduceFlag) {
						reduceFlag = true;
						reduceCache();
					}
				} catch (Exception e) {
				...
				try {
					Thread.sleep(5000); // 5 seconds
					MetaLog.info("totalCacheNum:" + totalCacheNum);
					MetaLog.info("maxINodeNum:" + maxINodeNum);
				} catch (InterruptedException ie) {
				}
			}
		}
	...
	void reduceCache() throws Exception {

		// reduce 20% num of node
		int num = (int) (totalCacheNum * 0.2);
		INode inode = null;
		while (num > 0) {
			inode = cacheLeast.previous;
			cacheLeast.previous = inode.previous;
			MetaLog.info("Reduce the inode:" + new String(inode.name));
			// inode is file
			if (!inode.isDirectory()) {
				INodeFile inodefile = (INodeFile) inode;
				inode.parent.removeChildFromMem(inodefile);
				for (int i = 0; i < inodefile.blocks.length; i++)
					blocksMap.removeBlockFromMem(inodefile.blocks[i]);
			}
			// inode is directory
			else {
				INodeDirectory dir = (INodeDirectory) inode;
				dir.cacheChild2Parent();
				inode.parent.removeChildFromMem(dir);
		...
		cacheLeast.previous.next = cacheLeast;
