...
class INodeFileUnderConstruction extends INodeFile {
	...
	INodeFileUnderConstruction(PermissionStatus permissions, short replication,
	 ...
	 */
	/*
	public INodeFileUnderConstruction(int diskIndex, byte[] name,
			short blockReplication, long modificationTime,
	...
	INodeFile convertToInodeFile() {
		INodeFile obj = new INodeFile(getPermissionStatus(), getBlocks(),
				getReplication(), getModificationTime(), getModificationTime(),
	...
	synchronized void setLastBlock(BlockInfo newblock,
		...
		if (blocks == null || blocks.length == 0) {
			throw new IOException(
					"Trying to update non-existant block (newblock=" + newblock
							+ ")");
		}
		BlockInfo oldLast = blocks[blocks.length - 1];
		if (oldLast.getBlockId() != newblock.getBlockId()) {
			// This should not happen - this means that we're performing
			// recovery
		...
		if (oldLast.getGenerationStamp() > newblock.getGenerationStamp()) {
			NameNode.stateChangeLog.warn("Updating last block " + oldLast
					+ " of inode " + "under construction " + this
					+ " with a block that " + "has an older generation stamp: "
					+ newblock);
		}
	...
	void assignPrimaryDatanode() {
		...
		for (int i = 1; i <= targets.length; i++) {
			...
			if (targets[j].isAlive) {
				...
				primary.addBlockToBeRecovered(blocks[blocks.length - 1],
						targets);
