...
import org.apache.hadoop.fs.Tools;
...
public class PermissionStatus implements Writable {
   ...
   */
  	public void  writePermission(FileChannel fc) throws IOException{
		Tools.writeFString(fc, username);
		Tools.writeFString(fc, groupname);
		Tools.writeShort(fc, permission.toShort());
	}
	public void readPermission(FileChannel fc)throws IOException{
		username=Tools.readFString(fc);
		groupname=Tools.readFString(fc);
		if(permission==null)
			permission=new FsPermission(Tools.readShort(fc));
		else
		permission.fromShort(Tools.readShort(fc));
	}
